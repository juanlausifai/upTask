...
if (tareasProceso) {
    tareasProceso.addEventListener('click',(e)=>{
        ...
        if (e.target.classList.contains('fa-arrow-right')) {
            ...
            .then(function(respuesta){
                if(respuesta.status===200){
                    functionPasarTareaFinalizada(parrafoText, idTarea,nombreEncargado,fechaVencimiento,idUsuarioLi);
                    li.remove();
        ...
        if (e.target.classList.contains('fa-arrow-left')) {
            ...
            if (idUsuarioLi!==inputIdUsuario) {
              ...
              //request hacia /tareas/:id
              const url=`${location.origin}/tareasretroceder/${idTarea}`;
              
              axios.patch(url,{idTarea})
              .then(function(respuesta){
                  if(respuesta.status===200){
                      functionPasarTareaEnEspera(parrafoText, idTarea,nombreEncargado,fechaVencimiento,idUsuarioLi);
                      li.remove();
                      actualizarAvance();
                  }
              });
         ...
         if (e.target.classList.contains('fa-play')) {
            ...
            //request hacia /tareasejecutando/:id
            const url=`${location.origin}/tareasejecutando/${idTarea}`;
...
function functionPasarTareaFinalizada(parrafoText, id,nombreEncargado,fechaVencimiento,idUsuarioLi) {
    
    const tareasProceso=document.querySelector('.listado-finalizado');
